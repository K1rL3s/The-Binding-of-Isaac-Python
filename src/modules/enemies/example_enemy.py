...
from src.modules.base_classes import BaseTear, MovingEnemy, ShootingEnemy
from src.modules.characters.main_hero import Player
from src.modules.entities.tears.example_tear import ExampleTear
...
class ExampleEnemy(MovingEnemy, ShootingEnemy):
    ...
    def move_back(self, rect: pg.Rect):
        ...
        if (self.rect.centerx < centerx and self.vx > 0) or (
            self.rect.centerx > centerx and self.vx < 0
        ):
            self.set_speed(0, self.speed if self.vy > 0 else -self.speed)
        if (self.rect.centery > centery and self.vy < 0) or (
            self.rect.centery < centery and self.vy > 0
        ):
    ...
    def draw_stats(self, screen: pg.Surface):
        ...
        text3 = font.render(
            f"vx={round(self.vx / CELL_SIZE, 3)}",
            True,
            (100, 255, 100),
        )
        text4 = font.render(
            f"vy={round(self.vy / CELL_SIZE, 3)}",
            True,
            (100, 255, 100),
        )
        h = text.get_height()
