...
class HeroStats:
    ...
    def draw_hearts(self):
        self.hearts_surface = pg.Surface(
            (HeroStats.size * 10, HeroStats.size * 2),
            pg.SRCALPHA,
            32,
        )
        red_hp, max_hp, blue_hp, black_hp = (
            self.main_hero.red_hp,
            self.main_hero.max_red_hp,
            self.main_hero.blue_hp,
            self.main_hero.black_hp,
        )
        x, y = 0, 0
    ...
    def draw_other(self):
        self.other_surface = pg.Surface(
            (HeroStats.size_s * 4, HeroStats.size_s * 3),
            pg.SRCALPHA,
            32,
        )
        ...
        self.font.place_text(
            self.other_surface,
            str(self.main_hero.count_money),
            (0, 0),
            (HeroStats.size_s * 2, 0),
        )
        self.font.place_text(
            self.other_surface,
            str(self.main_hero.count_bombs),
            (0, 0),
            (HeroStats.size_s * 2, HeroStats.size_s),
        )
        self.font.place_text(
            self.other_surface,
            str(self.main_hero.count_key),
            (0, 0),
    ...
    def render(self, screen: pg.Surface):
        screen.blit(
            self.other_surface,
            (
                MINIMAP_WIDTH + HeroStats.size_s * 3,
                (STATS_HEIGHT - self.other_surface.get_height()) // 2,
            ),
        )
        screen.blit(
            self.hearts_surface,
