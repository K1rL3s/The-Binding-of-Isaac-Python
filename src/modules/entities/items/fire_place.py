...
from src.modules.base_classes import DestroyableItem, MoveSprite, ShootingEnemy
...
class FireTextures(FireImage):
    fires_100 = [
        FireImage.fires.subsurface(
            0,
            y * CELL_SIZE,
            FireImage.fires.get_width(),
            CELL_SIZE,
        )
        for y in range(2)
    ]
    ...
    fires_wood = [
        (
            load_image("textures/room/firewoods.png").subsurface(
                x * CELL_SIZE,
                0,
                CELL_SIZE,
                CELL_SIZE,
    ...
    def set_image(self):
        for i, fire_type in enumerate(FirePlacesTypes):
            if fire_type == self.fire_type:
                self.stages = [
                    FireTextures.fires_100[i],
                    FireTextures.fires_67[i],
                    FireTextures.fires_33[i],
                    [pg.Surface((0, 0))],
                ]
                break
        assert self.stages
