...
class Door(BaseItem, DoorTextures):
    ...
    def set_rect(
        ...
        if self.to_room_type == consts.RoomsTypes.SECRET:
            image: pg.Surface = getattr(
                DoorTextures,
                f"{self.floor_type.value}_blow_{self.direction}",
            )
    ...
    def open(self, with_sound: bool = True, with_key: bool = False):
        ...
        if self.to_room_type == consts.RoomsTypes.SECRET:
            return
        if (
            self.to_room_type in (consts.RoomsTypes.SHOP, consts.RoomsTypes.TREASURE)
            and not with_key
        ):
            return
    ...
    def collide(self, other: MoveSprite):
        ...
        if self.collidable and (
            not isinstance(other, Player)
            or self.to_room_type
            not in (consts.RoomsTypes.SHOP, consts.RoomsTypes.TREASURE)
        ):
            return
