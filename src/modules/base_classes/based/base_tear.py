...
class BaseTear(MoveSprite, PopsImage):
    ...
    impacts: list[pg.mixer.Sound] = [
        load_sound(f"sounds/tear_impact{i}.mp3") for i in range(1, 4)
    ]
    ...
    def update(self, delta_t: float):
        ...
        if (
            math.hypot(self.start_x - self.rect.x, self.start_y - self.rect.y)
            > self.max_distance * CELL_SIZE
        ) or (self.max_lifetime and self.lifetime_ticks >= self.max_lifetime):
            self.destroy()
    ...
    def destroy_animation(self, delta_t: float):
        if not self.animation:
            raise SyntaxError(
                "Нет анимации при наследовании от BaseTear / "
