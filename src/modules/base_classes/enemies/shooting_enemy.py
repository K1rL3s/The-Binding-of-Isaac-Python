...
from src.modules.base_classes.enemies.base_enemy import BaseEnemy
...
class ShootingEnemy(BaseEnemy):
    ...
    def shot(self) -> bool:
        ...
        if (
            distance > self.shot_max_distance * CELL_SIZE or distance == 0
        ):  # Стреляет тогда, когда уже вплотную, ну хз.
            return False

        self.shot_ticks = 0
        vx = self.shot_max_speed * dx / distance + getattr(
            self,
            "vx",
            0,
        )  # Учёт собственной скорости
        vy = self.shot_max_speed * dy / distance + getattr(
            self,
            "vy",
            0,
        )  # Учёт собственной скорости
