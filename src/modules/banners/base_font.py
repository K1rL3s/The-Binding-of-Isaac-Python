...
class BaseFont:
    ...
    def __init__(
        ...
        assert len(self.letters) == len(
            alphabet,
        ), "Неверно введены буквы или неверно указан путь до шрифта"
    ...
    def write_text(self, text: str) -> pg.Surface:
        ...
        for i, symb in enumerate(text):
            try:
                surface.blit(self.letters[self.alphabet.index(symb)], (i * width, 0))
            except IndexError:
                raise IndexError(
                    f'В шрифте с буквами "{self.alphabet}" нет буквы "{symb}',
                )
