...
class TexturesHeroes:
    ...
    textures: dict[str, dict] = {
        name: {
            "body": {
                "DOWN": [
                    crop(load_image(f"textures/heroes/body/forward/{name}_{i}.png"))
                    for i in range(10)
                ],
                "LEFT": [
                    crop(load_image(f"textures/heroes/body/left/{name}_{i}.png"))
                    for i in range(10)
                ],
                "RIGHT": [
                    crop(load_image(f"textures/heroes/body/right/{name}_{i}.png"))
                    for i in range(10)
                ],
                "UP": [
                    crop(load_image(f"textures/heroes/body/up/{name}_{i}.png"))
                    for i in range(10)
                ],
...
class ParamsHeroes(TexturesHeroes):
    ...
    settings_head: list[int] = [
        pg.K_LEFT,
        pg.K_RIGHT,
        pg.K_UP,
        pg.K_DOWN,
    ]  # настройки поворота головы

    directions_head = {
        settings_head[0]: "LEFT",
    ...
    body_images_dict: (
        dict | None
    )  # Словарь текстурок движения тела. Ключ - Направление, значение - список изображений.
    head_images_dict: (
        dict | None
    )  # Словарь текстурок поворота головы. Ключ - Направление, значение - изображение.
    characterizations: dict[str, dict] = {
        "isaac": {"hp": 8, "speed": 4, "damage": 5, "is_flying": False, "offset": 10},
        "cain": {"hp": 6, "speed": 6, "damage": 4, "is_flying": False, "offset": 10},
        "lost": {"hp": 2, "speed": 4, "damage": 3, "is_flying": True, "offset": 18},
...
class Head(pg.sprite.Sprite):
    ...
    def __init__(
        ...
        self.player_speed: tuple[float, float] = (
            0,
            0,
        )  # текущая скорость персонажа(для отклонения пули при беге)
        ...
        self.last_name_direction: str = (
            "DOWN"  # последнее направление головы (по сути, последняя нажатая кнопка)
        )
        self.on_directions: list[str, ...] = (
            []
        )  # список активных направлений (по сути, список нажатых кнопок)
    ...
    def settings_vx_vy_tear(self):
        ...
        if self.is_rotated:
            if self.last_name_direction in ["LEFT", "RIGHT"]:
                self.vx_tear = (
                    self.shot_speed
                    if self.last_name_direction == "RIGHT"
                    else -self.shot_speed
                )
                # if self.player_speed[1] != 0:
                self.vy_tear += (
                    self.player_speed[1] * 0.3
                )  # константа выведена практически
            elif self.last_name_direction in ["UP", "DOWN"]:
                self.vy_tear = (
                    self.shot_speed
                    if self.last_name_direction == "DOWN"
                    else -self.shot_speed
                )
                # if self.player_speed[0] != 0:
                self.vx_tear += (
                    self.player_speed[0] * 0.3
                )  # константа выведена практически
    ...
    def set_directions(self, direction: str, is_down: bool):
        (
            self.on_directions.append(direction)
            if is_down
            else self.on_directions.remove(direction)
        )
...
class Player(MoveSprite):
    ...
    hurt_sounds: list[pg.mixer.Sound] = [
        load_sound(f"sounds/isaac_hurt{i}.mp3") for i in range(1, 4)
    ]
    ...
    def __init__(self, name: str):
        ...
        self.indexes = {
            ...
            "RIGHT": 0,
            "UP": 0,
        }  # кол-во кадров, прошедших в определённую сторону

        self.tears = pg.sprite.Group()  # слёзы
        self.max_red_hp, self.max_speed, damage, self.is_flying, self.offset = (
            self.params_hero.get_characters(name)
        )
        self.head = Head(damage, self.tears, self.params_hero)  # голова персонажа

        self.collide_groups: tuple[pg.sprite.AbstractGroup, ...] | None = (
            None  # группы, в которые может врезаться ГГ
        )
        ...
        # флаги направления
        self.last_name_direction: str = (
            "DOWN"  # в прошлый кадр анимация была в эту сторону
        )
        self.move_last_direction: str | None = (
            None  # в прошлый кадр ГГ двигался в эту сторону
        )
    ...
    def animating(self):
        ...
        if self.count_cadrs == 0:
            ...
            if self.is_move:
                self.image = self.params_hero.body_images_dict[last_name][
                    self.indexes[last_name]
                ]
            else:
                self.image = self.params_hero.body_images_dict["DOWN"][0]
    ...
    def settings_move_speed(self, delta_t: float):
        ...
        if (
            (self.flag_move_up or self.flag_move_down)
            and not (self.flag_move_up and self.flag_move_down)
            and (self.flag_move_left or self.flag_move_right)
            and not (self.flag_move_left and self.flag_move_right)
            and not (self.x_collide or self.y_collide)
        ):
        ...
        if (direction == Moves.RIGHT and self.vx > 0) or (
            ...
            if self.vy:
                self.vy = (-1 if self.vy < 0 else 1) * (
                    abs(self.vy) + abs(self.vx * 0.7)
                )
            self.vx = 0

        elif (direction == Moves.UP and self.vy < 0) or (
            direction == Moves.DOWN and self.vy > 0
        ):
            self.move_last_direction = "LEFT" if self.vx < 0 else "RIGHT"
            if self.vx:
                self.vx = (-1 if self.vx < 0 else 1) * (
                    abs(self.vx) + abs(self.vy * 0.7)
                )
            self.vy = 0
...
class Soul(pg.sprite.Sprite):
    ...
    def move(self, delta_t):
        ...
        """
        if (
            self.xy_pos[0] <= self.start_x_pos - CELL_SIZE // 4
            or self.xy_pos[0] >= self.start_x_pos + CELL_SIZE // 4
        ):
            self.vx *= -1
        self.xy_pos = (
            self.xy_pos[0] + self.vx * delta_t,
            self.xy_pos[1] + self.vy * delta_t,
        )
