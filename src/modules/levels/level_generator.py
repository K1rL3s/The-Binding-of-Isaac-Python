...
from src.consts import Moves, RoomsTypes
...
def all_rooms_have_path_to_start(
    rooms: list[list[RoomsTypes | str]],
    *,
    ignore_secret: bool = True,
) -> bool:
    """
...
def has_path_to_start(
    start_pos: tuple[int, int],
    rooms: list[list[RoomsTypes | str]],
    *,
    ignore_secret: bool = True,
    graph: dict[tuple[int, int], list[tuple[int, int]]] = None,
) -> bool:
    """
    ...
    end_pos = (
        math.ceil(map_width / 2) - 1,
        math.ceil(map_height / 2) - 1,
    )
    if graph is None:
        graph = make_neighbors_graph(rooms, ignore_secret=ignore_secret)
...
def set_secret_room(rooms: list[list[RoomsTypes | str]]) -> bool:
    ...
    for neighbors_rooms in range(4, 1, -1):
        secrets = [
            room
            for room in graph
            if len(graph[room]) >= neighbors_rooms
...
def set_special_rooms(rooms: list[list[RoomsTypes | str]]) -> bool:
    ...
    solo = [
        room
        for room in graph
        if len(graph[room]) == 1 and rooms[room[1]][room[0]] == RoomsTypes.DEFAULT
    ]
    if len(solo) < 3:
        return False
    ...
    boss_x, boss_y = max(
        solo,
        key=lambda c: math.sqrt((spawn_x - c[0]) ** 2 + (spawn_y - c[1]) ** 2),
    )
...
def generate_level(
    ...
    assert (
        room_numbers < map_width * map_height - 3
    )  # Возможно сгенерировать все комнаты
