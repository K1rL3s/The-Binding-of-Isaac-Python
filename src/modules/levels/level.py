...
from src.modules.animations.moving_room_animation import MovingRoomAnimation
from src.modules.characters.main_hero import Player
from src.modules.levels.level_generator import generate_level
from src.modules.levels.room import Room
from src.utils.graph import get_neighbors_coords, valid_coords
...
class Level:
    ...
    def setup_level(self):
        ...
        assert (
            ...
            and self.current_room.room_type == consts.RoomsTypes.SPAWN
        )
    ...
    def get_doors(
        ...
        cur_x: int,
        cur_y: int,
    ) -> list[tuple[consts.DoorsCoords, consts.RoomsTypes]]:
        """
        ...
        for room_x, room_y in coords:
            ...
            assert direction is not None, "Не удалось получить расположение двери"

            if (
                self.level_map[cur_y][cur_x]
                in (
    ...
    def moving_room_animation(
        self,
        from_room: Room,
        to_room: Room,
        direction: consts.Moves,
    ):
        """
